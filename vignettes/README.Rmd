...
3. Execute [dxsmall_simulation.R]() and [dxsmall_simulation.sh]() to generate multiple .rds files, containing down-sampling simulation results based on the parameters from the Mclust fit model. Ensure these files are saved in the same directory. Afterwards, utilize [merge_results.R]() to aggregate all files and generate [module2_dxsmall_norm_all_tests.rds]().
