...
### Module 1
To generate the power and false positive results of the simulation study, execute [Module1.R]("https://github.com/mao-ding/bifurcatoR/blob/main/vignettes/Simulation_Study/Module_1/scripts/Module1.R") The study includes 7 tests: "Levene," "Permutations (Gini)," "Permutations (MAD)," "Permutations (SD)," "ANOVA," "Non-parametric ANOVA," and "Permutations (Raw)."
...
After the completion of Module_1 execution, it produced multiple raw results .rds files, which are stored within the same directory. To consolidate these files with identical structure in the same folder, we employ [merge_results.R]("https://github.com/mao-ding/bifurcatoR/blob/main/vignettes/Simulation_Study/Module_1/scripts/merge_results.R"). Simply adjust the working directory and the folder for result storage as required. This process yields [module1_n_w_7_tests_sim(1000).rds](https://github.com/mao-ding/bifurcatoR/blob/main/vignettes/Simulation_Study/Module_1/results/module1_n_w_7_tests_sim(1000).rds) and [module1_n_w_7_tests_sim(1000).csv]()
...
After the execution of Module_2 is complete, the same [merge_results.R]("https://github.com/mao-ding/bifurcatoR/blob/main/vignettes/Simulation_Study/Module_1/scripts/merge_results.R") script is utilized to combine the raw .rds files. Ensure to adjust the saving directory according to where you want the file to be stored.

Specifically, for normal, Weibull, and lognormal distributions, merge the raw results from [Module2_Norm_Weib_Distribution.R], [Module2_Norm_Weib_Distribution_mixR.R], and [Module2_LNorm_Distribution_mixR.R] to generate [module2_n_w_9_tests_sim(1000).rds] (https://github.com/mao-ding/bifurcatoR/blob/main/vignettes/Simulation_Study/Module_2/results/module2_n_w_9_tests_sim(1000).rds) and [module2_n_w_9_tests_sim(1000).csv]()

For the beta distribution, merge the raw results generated from [Module2_Beta_Distribution.R] to produce [module2_b_all_tests_sim(1000).rds](https://github.com/mao-ding/bifurcatoR/blob/main/vignettes/Simulation_Study/Module_2/results/module2_b_all_tests_sim(1000).rds) and [module2_b_all_tests_sim(1000).csv]()

We created an overall combined results .xlsx file [Module_1_2_Results.xlsx]() using the three .csv file above through Microsoft Excel.

## dxsmall Data Analysis
1. Execute [Table_1_A_dxsmall.R]() to generate the table displaying the characteristics of the TARGET pAML and UK Twins cohorts, labeled as [Table 1. TARGET pAML and UK Twins cohort characteristics]().

2. Utilize [Figure_4_A_B_C_D.R]() to produce the following figures: [fig_4_A.png](), [fig_4_B.png](), [fig_4_C.png](), [fig_4_D.png]().

3. Execute [dxsmall_simulation.R](https://github.com/mao-ding/bifurcatoR/blob/main/vignettes/dxsmall/scripts/dxsmall_simulation.R) and [dxsmall_simulation.sh](https://github.com/mao-ding/bifurcatoR/blob/main/vignettes/dxsmall/scripts/dxsmall_simulation.sh) to generate multiple .rds files, containing down-sampling simulation results based on the parameters from the Mclust fit model. Ensure these files are saved in the same directory. Afterwards, utilize [merge_results.R](https://github.com/mao-ding/bifurcatoR/blob/main/vignettes/Simulation_Study/Module_1/scripts/merge_results.R) to aggregate all files and generate [module2_dxsmall_norm_all_tests.rds]().
