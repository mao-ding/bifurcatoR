...
#results = rbindlist(mclapply(1:nrow(params_grid), function(x) process_sims_n_w(x), mc.cores = total_cores))
results = process_sims_n_w(as.integer(args[1]))

time.taken <- end.time - start.time
time.taken

saveRDS(results, paste('module_2_n_w_results', args, sep="_"),)